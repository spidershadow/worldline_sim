...
Run example
-----------
python -m fixed_retro_run --patterns uap,tech,rng,events \
       --runs-per-tstar 5 --tstar-range 2030 2040 --leak-lambda 0.2 --leak-tau0 20 \
       --max-tau 70 --plot
"""
...
def backfill_timeline(patterns: List[Pattern], t_star: int, *, max_tau: int) -> Timeline | None:
    ...
    for y in reversed(range(start_year, t_star)):
        ...
        for p in patterns:
            ...
            if p.name == "rng":
                # REMOVED THE EXTRA 0.01 SCALING FACTOR HERE
...
def build_argparser():
    ...
    p.add_argument("--debug", action="store_true", help="Enable extra debugging output")
    return p
...
def main(argv=None):  # noqa: D401
    ...
    for t_star in range(lo, hi + 1):
        ...
        for k in range(args.runs_per_tstar):
            ...
            if tl is None:
                # Debug trajectory rejection
                if args.debug and k < 5:
                    print(f"[DEBUG-REJECT] T*={t_star} k={k} - Timeline didn't satisfy constraints")
                continue
    ...
    if args.plot and weighted_counts:
        ...
        img_path = Path(__file__).parent / "fixed_retro_tstar_hist.png"
        plt.tight_layout()
