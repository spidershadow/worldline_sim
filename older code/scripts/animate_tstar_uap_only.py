#!/usr/bin/env python3
"""Animate evolving posterior of T* using a two‑sided window‑fit score (UAP only).

This is a specialized version that focuses only on the UAP pattern and uses
a more tolerant approach to constraints. For each candidate Singularity year T*,
we generate a retro-backfilled timeline and compute a symmetry error between
past and future windows around T* (values at year T*-τ should match those at T*+τ).
The overall weight for that T* is an exponentially decayed function of the
aggregated squared differences.
...
Example
-------
python scripts/animate_tstar_uap_only.py \
       --tstar-range 2030 2100 \
...
from worldline_sim.patterns.uap import UapPattern
...
def save_timeline_data(tl, patterns: List, t_star: int, output_dir: Path, run_idx: int) -> None:
    ...
    print(f"[UAP] Saved timeline to {csv_path}")
    return csv_path
...
def build_argparser():
    p = argparse.ArgumentParser(description="Animate posterior of T* using window‑fit scoring (UAP only)")
    p.add_argument("--patterns", default="uap", help="comma‑list of patterns to load (default: 'uap')")
    ...
    p.add_argument("--correction-factor", type=float, default=0.7, help="correction factor for patterns")
    p.add_argument("--noise-range", type=float, default=0.1, help="noise range as fraction of base value")
    p.add_argument("--tolerance", type=float, default=20.0, help="tolerance for pattern constraints")
...
def main(argv=None):
    ...
    output_prefix = args.output_prefix or f"uap_tstar_{timestamp}"
    ...
    print(f"[UAP] Using base seed: {seed}")
    ...
    for run_idx in range(args.runs):
        ...
        print(f"[UAP] Starting run {run_idx + 1}/{args.runs} with seed {run_seed}")
        ...
        for t_idx, t_star in enumerate(t_candidates):
            ...
            if tl is None:
                print(f"[UAP] Invalid timeline for T*={t_star}, run {run_idx}")
                continue
        ...
        if (run_idx + 1) % 10 == 0 or run_idx == args.runs - 1:
            print(f"[UAP] Completed run {run_idx + 1}/{args.runs}")
            
            # Save histogram for this checkpoint
            if prob.sum() > 0:
                plt.figure(figsize=(10, 5))
                plt.bar(t_candidates, prob, color="tab:blue")
                plt.xlabel("Candidate Singularity Year T*")
                plt.ylabel("Posterior probability (normalised)")
                plt.title(f"T* Distribution after {run_idx + 1} runs (UAP only)")
                ...
                plt.close()
                print(f"[UAP] Saved checkpoint plot to {checkpoint_file}")
    ...
    print(f"[UAP] Saved error statistics to {error_stats_file}")
    ...
    if not frames or all(f.sum() == 0 for f in frames):
        print("[UAP] WARNING: No valid weights found. Using uniform distribution.")
        frames = [np.ones_like(t_candidates) / len(t_candidates) for _ in range(args.runs)]
    
    bar_container = ax.bar(t_candidates, frames[0], color="tab:blue")
    ...
    ax.set_title("Evolution of posterior over T* (UAP pattern only)")
    ...
    def update(frame_idx):
        ...
        for rect, h in zip(bar_container, probs):
            rect.set_height(h)
        ax.set_title(f"Evolution of posterior over T* (UAP only)  |  after {frame_idx + 1} runs")
    ...
    print(f"[UAP] Animation saved to {output_mp4}")
    
    # Save final frame as a static image
    final_frame = animation_dir / f"{output_prefix}_final.png"
    plt.figure(figsize=(10, 5))
    plt.bar(t_candidates, frames[-1], color="tab:blue")
    plt.xlabel("Candidate Singularity Year T*")
    plt.ylabel("Posterior probability (normalised)")
    plt.title(f"Final T* Distribution (UAP only, after {args.runs} runs)")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(final_frame, dpi=150)
    plt.close()
    print(f"[UAP] Final frame saved as {final_frame}")
